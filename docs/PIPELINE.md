...
**Valid actions:** `scrape_selenium`, `parse`, `matrix`, `train_super`,
`score_super`, `train_unsuper`, `validate`.
...
command line, the CLI runs them in its built-in order. `scrape_selenium` runs
first; then the `ALL_ACTION_SEQUENCE`:
