...
onward, each category is routed into exactly one of **four branches**, and the
...
| **Bow weapon DPS (single)** | `Bow` | 1 global model |
...
<!-- Retired currency reference: | `divine_exalt` / `chaos_exalt` / `annul_exalt` | `197` / `19` / `107` | Currency → exalt ratios | -->
...
### 2.7 Craft Oracle intrinsic-affix audit

The parse stage also observes structured ordinary `explicitMods[].mods[]`
contributors and atomically updates
`db/super_models/craft_oracle_affix_catalog.json`. This sidecar never changes
the aggregated DataFrame or feature matrices.

After parsing, each selected category emits a Craft Oracle audit summary with
accepted item/contributor counts, pattern coverage, retained tier/identity
counts, usable structured-evidence rate, and skipped
legacy/special/missing/invalid-tier counts. Contributor identities are retained
only from unambiguous single-contributor parents with a consistent canonical
pattern signature. `python -m poe2trade.db` captures this output in the run's
`parse.txt`. Partial runs may be reported as not release-ready; the supervised
artifact validator later enforces the production evidence thresholds for every
...
<!-- Retired currency reference:    `annul × annul_exalt`; unknown currency → `0.0`. -->
...
### 3.4 Bow weapon-DPS branch

Condition: category token resolves to `bow`.

Detailed algorithm README: [`WEAPON_FEATURES.md`](WEAPON_FEATURES.md).

- Raw weapon properties are parsed into physical/fire/cold/lightning/chaos
  damage ranges, attack speed, and critical hit chance.
- Socket/rune effects are removed from displayed weapon damage before DPS is
  modeled: direct `% increased Physical Damage` runes and direct `Adds A to B
  <type> Damage` runes are deflated from the corresponding damage type.
- Final model features are `dps_total`, one `dps_<type>` feature for each damage
  type, `crit_chance`, `extra_sockets`, and the usual implicit/enchant/explicit
  mod features. Rune mods are not emitted as standalone features.

### 3.5 Category → branch resolution table
...
| Bow | Bow weapon DPS | No | `dps_total`, per-type DPS, `crit_chance`; socket/rune deflation |
...
current `divine_exalt/chaos_exalt` and blocks on Enter so you can
...
| --- | --- | --- | --- |
| `matrix_utils` | `belt`/`ring`/`amulet` in path | `jewelry & "belt" in stem` | jewel(s)/sceptre/staff/wand/quiver; Bow has its own DPS branch |
| `train_utils` | `ring`/`amulet` only | `belt & not ring/amulet` | jewel(s)/sceptre/staff/wand/quiver/bow |
| `score_super_utils` | `ring`/`amulet` (separate belt fn) | `belt & not ring/amulet` | + quiver/bow, dynamic from `jewel_list` |

All converge on: armour → segmented; belt/ring/amulet/simple/bow → single global.
