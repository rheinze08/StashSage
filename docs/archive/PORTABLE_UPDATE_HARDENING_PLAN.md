...
> Archived 2026-09-03. Completed work and still-relevant follow-ups were
> consolidated into `docs/IMPLEMENTATION_NOTES.md`. Retained here as historical
> design context.
...
- [x] **`-Republish` app-code-change guard.** `-Republish` reuses the existing
  version, and installed clients only download the app package when the
  manifest `app_version` is strictly newer (`app_update_available` ->
  `is_newer_version`), so a republish can only redeliver hash-checked data
  assets - any app-code change (e.g. new stash-scrape categories in
  `scrape_stash_utils.py`) silently never reaches installed clients. *Done:
  `Update-Release.ps1` always warns in the `-Republish` branch that clients
  will not receive the new exe/UI, and `Get-RepublishCodeChanges` diffs the
  release tag against the working tree under `poe2trade/` (excluding the
  `db/`/`generated/`/`logs/` asset trees), failing the release when app code
  changed unless `-ForceRepublish` is passed for a deliberate asset-only
  re-upload. Because `gh release create` only tags the public repo, a
  successful normal release now also creates the local `v<version>` tag in the
  poe2trade checkout so both this guard and the monotonicity guard have a
  baseline. Covered by `tests/test_release_contract.py`.*
