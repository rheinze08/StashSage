...
The `dps_*` values used by the model are intentionally normalized: displayed
quality and socket/rune effects are removed so training and inference use the
same representation. They are not the weapon's actual tooltip DPS.

User-facing surfaces use `compute_displayed_weapon_dps_features` instead. It
multiplies each displayed, already-augmented damage-range average by the
displayed attacks per second. Overlay matrices likewise keep normalized DPS in
the model matrix and actual DPS in the comparison/display matrix.

The two representations are handed between stages by mutation, not by return
value: `deflator_and_normaliser` leaves *displayed* DPS in `dps_*` so the serve
API can build its target payload, and stashes the model values under `_model_*`;
`cleanup_unused_features` promotes them back before inference. A caller that
runs the first without the second scores on displayed DPS with no other symptom,
so `test_model_dps_staging_contract_between_normaliser_and_cleanup` pins the
handshake.

This weapon branch applies only to the categories in `DPS_WEAPON_CATEGORIES`.
Wands, sceptres, and staves are trained categories that are **not** in that set:
they get no `dps_*` features, and `BOW_RAW_STAT_COLUMNS` is dropped
unconditionally, so their base damage and attack speed reach the model in no
form at all. Their `Adds # to # X Damage` explicits survive as ordinary direct
features instead, because the omitted-pattern skip is also gated on
`DPS_WEAPON_CATEGORIES`. That split is defensible for caster weapons whose value
sits in skill levels and spell damage, but it is a real difference in
representation rather than the uniform treatment the rest of this document
describes.
...
> **Known defect — needs a retrain to fix.** PoE applies weapon quality as a
> *local increased physical damage* modifier, and local increases stack
> **additively** with each other and with explicit `#% increased Physical
> Damage`. The displayed average is `base * (1 + (Q + explicit% + rune%) / 100)`,
> which the multiplicative denominator above does not invert. The resulting
> `dps_physical` is correct at 0% quality and drifts low as explicit physical
> percent rises — about −8% at 20% quality with 100% explicit, −10% at 150%.
> The error is therefore correlated with the item's main value driver rather
> than being constant noise, and two identical weapons at different quality map
> to different feature values. The same expression subtracts `rune_flat` after
> the division, although local flat damage is added before local increases
> apply. Correcting either changes the trained feature space, so the fix must
> ship with a retrain and a before/after comparison — see
> `docs/archive/PRODUCT_GAPS_AUDIT.md` item 1.
