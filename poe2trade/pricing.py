...
class CurrencyConversions:
    ...
    def lookup_model_training_snapshot(self, model_path: str | Path) -> Mapping[str, object] | None:
        ...
        A model-set can coexist with historical flat artifacts, so the
        sidecar beside the exact resolved model always wins over the flat
        conversion index.
        """
        artifact = Path(model_path)
        ...
        is_model_set_artifact = artifact.parent.parent.parent.name == "model_sets"
        if is_model_set_artifact:
            ...
            try:
                ...
                # Older model sets shipped the adjacent training sidecar
                # instead of a pricing manifest; retain that compatibility.
                pass
            else:
                return {
                    "conversions": {
        ...
        sidecar = artifact.with_suffix(".training.json")
        try:
            payload = json.loads(sidecar.read_text(encoding="utf-8"))
        except (OSError, ValueError, TypeError, json.JSONDecodeError):
            payload = None
        if isinstance(payload, Mapping) and isinstance(payload.get("conversions"), Mapping):
            result = dict(payload)
            result["is_mock"] = False
            result.setdefault("model_artifact", artifact.name)
            return result
        # A nested model's missing sidecars must not select a same-named model
        # from the flat index: that index has no league namespace.
        if is_model_set_artifact:
            return None
