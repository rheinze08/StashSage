...
from poe2trade.app.popup_escape import register_popup
...
def _open_stash_scrape_dialog() -> None:
    ...
    if embedded:
        ...
        dialog.focus_force()
    if not embedded:
        register_popup(root, dialog, dialog.destroy)
    ...
    def _handle_submit() -> None:
        ...
        if not embedded:
            register_popup(root, progress_dialog, _request_cancel_and_close)
...
def _open_fi_popup(cat: str | None, seg: str | None, model_type: str) -> None:
    ...
    register_popup(root, popup, popup.destroy)
...
def _show_stash_scrape_viewer(preselect: "str | os.PathLike[str] | None" = None) -> None:
    ...
    # Treeview tags colour entire rows. Paint only visible bucket cells and
    # forward pointer events so selection and wheel scrolling still work.
    bucket_cells: list[tk.Canvas] = []
    bucket_redraw = {"after_id": None}

    def _forward_bucket_event(event, sequence):
        options = {
    ...
    def _draw_bucket_cells():
        bucket_redraw["after_id"] = None
        for cell in bucket_cells:
            cell.place_forget()
        used = 0
        seen = set()
        for screen_y in [*range(0, tree.winfo_height(), 23), tree.winfo_height() - 1]:
            row_id = tree.identify_row(screen_y)
            if not row_id or row_id in seen:
                continue
            seen.add(row_id)
            bounds = tree.bbox(row_id, "bucket_label")
            label = tree.set(row_id, "bucket_label")
            colour = _BUCKET_COLOURS.get(label.strip().lower())
            if not bounds or not colour:
                continue
            x, y, width, height = bounds
            left, right = max(0, x), min(tree.winfo_width(), x + width)
            if right <= left:
                continue
            if used == len(bucket_cells):
                cell = tk.Canvas(tree, highlightthickness=0, borderwidth=0)
                for sequence in ("<Button-1>", "<ButtonRelease-1>", "<B1-Motion>", "<MouseWheel>"):
                    cell.bind(sequence, lambda event, seq=sequence: _forward_bucket_event(event, seq))
                bucket_cells.append(cell)
            cell = bucket_cells[used]
            used += 1
            cell.configure(background=colour)
            cell.delete("all")
            cell.create_text(
                x + width / 2 - left, height / 2, text=label,
                fill="#FFFFFF", font=("Segoe UI", 9),
            )
            cell.place(x=left, y=y, width=right - left, height=height)

    def _queue_bucket_redraw(*_args):
        if bucket_redraw["after_id"] is None:
            bucket_redraw["after_id"] = tree.after_idle(_draw_bucket_cells)

    def _stash_scroll(scrollbar, first, last):
        scrollbar.set(first, last)
        _queue_bucket_redraw()

    tree.configure(
        yscrollcommand=lambda first, last: _stash_scroll(vsb, first, last),
        xscrollcommand=lambda first, last: _stash_scroll(hsb, first, last),
    )
    tree.bind("<Configure>", _queue_bucket_redraw, add="+")
    tree.bind("<ButtonRelease-1>", _queue_bucket_redraw, add="+")
    tree.grid(row=0, column=0, sticky="nsew", padx=2, pady=2)
    vsb.grid(row=0, column=1, sticky="ns")
...
def _enable_overlay_escape(window) -> None:
    ...
    register_popup(root, window, _close_on_escape)
...
# The PoE client HWND last seen as the foreground window. A price overlay takes
# keyboard focus (so Escape reaches it), so a hotkey pressed while one is open
# sees the overlay, not the game, as foreground.
_last_foreground_poe_hwnd = 0


def _select_poe_window_rect(
    candidates: list[tuple[int, tuple[int, int, int, int], str]],
    foreground_hwnd: int,
    remembered_hwnd: int = 0,
    ...
    between those two surfaces; hotkeys are normally invoked while the game is
    the foreground window, so that HWND is the authoritative candidate.  When
    something else is foreground (our own overlay), the client that was last
    foreground is the next best evidence.
    """
    for preferred in (foreground_hwnd, remembered_hwnd):
        if not preferred:
            continue
...
def _poe_window_rect() -> tuple[int, int, int, int] | None:
    ...
    try:
        ...
        global _last_foreground_poe_hwnd
        if any(hwnd == foreground for hwnd, _rect, _title in candidates):
            _last_foreground_poe_hwnd = foreground
        selected = _select_poe_window_rect(candidates, foreground, _last_foreground_poe_hwnd)
        if selected is not None:
            logging.info(
                "PoE window candidates=%s foreground=%s remembered=%s selected=%s",
                [(hwnd, rect, title) for hwnd, rect, title in candidates],
                foreground,
                _last_foreground_poe_hwnd,
                selected,
            )
        return selected
    except Exception:
...
def _create_overlay_dialog(on_cancel) -> tuple[tk.Toplevel, ctk.CTkToplevel]:
    ...
    register_popup(root, win, on_cancel)
    backdrop.lift()
    return backdrop, win
...
def _present_dashboard_staged_v2(payload: Mapping[str, Any], request_id: int) -> None:
    ...
    def build_header() -> None:
        ...
        if isinstance(score, (int, float)):
            from poe2trade.app.prediction_presenter import _model_price_bar, _xgb_bar_text
            colour = _BUCKET_COLOURS.get(str(payload.get("bucket_label") or "").lower(), _BAR_COLOUR)
            _model_price_bar(header, _xgb_bar_text(payload), result, colour=colour).pack(
    ...
    def build_charts() -> None:
        ...
        if isinstance(summary, Mapping) and (summary.get("mean") is not None or summary.get("median") is not None):
            from poe2trade.app.prediction_presenter import _knn_bar_text, _model_price_bar
            _model_price_bar(header, _knn_bar_text(payload, summary), result, colour="#294354").pack(
                fill="x", padx=4, pady=(0, 4)
            )
        keep_cover_on_top()
    ...
    try:
        ...
        for record in similar_records:
            try:
                ...
                if price:
                    ...
                    price_labels.append(f"{float(price[1]):.0f}e")
            except Exception:
                continue
        ...
        if show_viz and prices:
            try:
                listing_context_buf = generate_prediction_vs_listings_chart(
                    display_value, prices, title="XGB & KNN Price Prediction Range"
                )
            except Exception:
        ...
        result_queue.put((
            "ok",
            {
                ...
                # presenter without reparsing the human-readable summary.
                "listing_prices": price_labels,
                ...
                "category_distribution_png": None,
                "distribution_png": (
...
def _show_validation_sample_picker(items: list[str], lock: threading.Lock) -> None:
    ...
    register_popup(root, popup, close)
...
def run_tkinter_app(cfg: Optional[dict] = None) -> None:
    ...
    ctk.CTkLabel(
        ...
        text=(
            "Type shortcuts as text without quotation marks. For example, enter "
            "ctrl+1; shortcuts are not captured automatically when you press keys. "
            "Leave a field blank to use its default, or enter off to disable it. "
            "On Windows, use num1 for the numeric keypad, such as ctrl+num1. "
            "Modifier keys must match exactly. Shortcuts pause while you edit these "
            "fields."
        ),
        text_color="#9AA7B4",
        font=_FONT_HELPER,
    ...
    prediction_section = _CollapsibleSection(
        main_content, "KNN Model", collapsed=True, on_toggle=_auto_resize_root
    )
    ...
    ctk.CTkLabel(
        ...
        text=(
            "Adjust how many similar items the KNN Model returns in the prediction "
            "overlay, and filter items by your preferred minimum or maximum prices. "
            "These filters can help exclude troll listings and unrealistic prices."
        ),
        text_color="#AAAAAA",
        font=_FONT_HELPER,
        wraplength=520,
