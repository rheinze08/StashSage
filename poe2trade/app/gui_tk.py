...
def _open_fi_popup(cat: str | None, seg: str | None, model_type: str) -> None:
    ...
    if isinstance(entry, dict):
        ...
        # Hide redundant '% increased block chance' for shield-like categories
        if str(cat_l) in ("shield", "buckler"):
            feats = [f for f in feats if str(f.get("name", "")).strip().lower() != "#% increased block chance"]
        # sort desc
        try:
    ...
    for r, (_, nbr) in enumerate(df.iterrows()):
        try:
            ...
            for order_idx, k in enumerate(noncore_candidates):
                # Skip Block-related rows for shield/buckler: now shown in core
                try:
                    lk = str(k).strip().lower()
                    if is_shield_like and (
...
def run_tkinter_app(cfg: Optional[dict] = None) -> None:
    ...
    general_section = _CollapsibleSection(
        root, "General Settings", collapsed=True, on_toggle=_auto_resize_root
    )
...
def _show_dashboard_overlay(
    ...
    *,
    ...
    try:
        ...
        if isinstance(entry, dict):
            feats = entry.get("features") or []
            # hide redundant block% for shields/bucklers
            if cat_l in ("shield", "buckler"):
                feats = [f for f in feats if str(f.get("name", "")).strip().lower() != "#% increased block chance"]
