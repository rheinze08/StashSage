...
def _bind_overlay_hotkey(custom: str | None) -> None:
    ...
    try:
        _overlay_hotkey_handle = keyboard.add_hotkey(
            desired, _handle_hotkey_super, suppress=True
        )
    ...
    if desired.lower() != DEFAULT_OVERLAY_HOTKEY:
        try:
            _overlay_hotkey_handle = keyboard.add_hotkey(
                DEFAULT_OVERLAY_HOTKEY, _handle_hotkey_super, suppress=True
            )
...
def _bind_filtered_overlay_hotkey(custom: str | None) -> None:
    ...
    try:
        _filtered_overlay_hotkey_handle = keyboard.add_hotkey(
            desired, _handle_hotkey_filtered, suppress=True
        )
    ...
    if desired.lower() != DEFAULT_FILTERED_OVERLAY_HOTKEY:
        try:
            _filtered_overlay_hotkey_handle = keyboard.add_hotkey(
                DEFAULT_FILTERED_OVERLAY_HOTKEY, _handle_hotkey_filtered, suppress=True
            )
...
def _bind_craft_potential_hotkey(custom: str | None) -> None:
    """Bind CraftOracle to its configured key with Ctrl+4 fallback."""
    global _craft_potential_hotkey_handle
    desired = (custom or "").strip() or DEFAULT_CRAFT_POTENTIAL_HOTKEY
    if _craft_potential_hotkey_handle is not None:
        try:
            keyboard.remove_hotkey(_craft_potential_hotkey_handle)
        except Exception:
            logging.debug("Previous Craft Potential hotkey removal failed", exc_info=True)
        _craft_potential_hotkey_handle = None
    try:
        _craft_potential_hotkey_handle = keyboard.add_hotkey(
            desired, _handle_hotkey_craft_potential, suppress=True
        )
        logging.info("Craft Potential hotkey bound to %s", desired)
    except Exception as exc:
        logging.error("Failed to bind Craft Potential hotkey %r: %s", desired, exc)
        if desired.lower() != DEFAULT_CRAFT_POTENTIAL_HOTKEY:
            try:
                _craft_potential_hotkey_handle = keyboard.add_hotkey(
                    DEFAULT_CRAFT_POTENTIAL_HOTKEY, _handle_hotkey_craft_potential, suppress=True
                )
            except Exception:
                logging.error("Could not bind fallback Craft Potential hotkey", exc_info=True)


def _bind_stash_scrape_hotkey(custom: str | None) -> None:
    """Bind the StashScrape workspace to its configured key."""
    global _stash_scrape_hotkey_handle
    desired = (custom or "").strip() or DEFAULT_STASH_SCRAPE_HOTKEY
    if _stash_scrape_hotkey_handle is not None:
        try:
            keyboard.remove_hotkey(_stash_scrape_hotkey_handle)
        except Exception:
            logging.debug("Previous StashScrape hotkey removal failed", exc_info=True)
        _stash_scrape_hotkey_handle = None
    try:
        _stash_scrape_hotkey_handle = keyboard.add_hotkey(
            desired, _handle_hotkey_stash_scrape, suppress=True
        )
        logging.info("StashScrape hotkey bound to %s", desired)
    except Exception as exc:
        logging.error("Failed to bind StashScrape hotkey %r: %s", desired, exc)
        if desired.lower() != DEFAULT_STASH_SCRAPE_HOTKEY:
            try:
                _stash_scrape_hotkey_handle = keyboard.add_hotkey(
                    DEFAULT_STASH_SCRAPE_HOTKEY, _handle_hotkey_stash_scrape, suppress=True
                )
            except Exception:
                logging.error("Could not bind fallback StashScrape hotkey", exc_info=True)
...
def save_and_reload() -> None:
    ...
    state.config.update(
        ...
        stash_scrape_hotkey=(
            stash_scrape_hotkey_entry.get().strip()
            if stash_scrape_hotkey_entry else ""
        ),
        craft_potential_hotkey=(
            craft_potential_hotkey_entry.get().strip()
            if craft_potential_hotkey_entry
            else ""
        ),
    )
    _apply_price_filter(state.config)
    ...
    _bind_stash_scrape_hotkey(state.config.get("stash_scrape_hotkey"))
    _bind_craft_potential_hotkey(state.config.get("craft_potential_hotkey"))
    _auto_resize_root()
    messagebox.showinfo(
        "StashSage",
...
def run_tkinter_app(cfg: Optional[dict] = None) -> None:
    ...
    def _hotkey_summary_text() -> str:
        return (
            ...
            f" | Overlay: {(state.config.get('custom_hotkey') or DEFAULT_OVERLAY_HOTKEY)}"
            f" | Filtered: {(state.config.get('filtered_overlay_hotkey') or DEFAULT_FILTERED_OVERLAY_HOTKEY)}"
            f" | StashScrape: {(state.config.get('stash_scrape_hotkey') or DEFAULT_STASH_SCRAPE_HOTKEY)}"
            f" | Craft: {(state.config.get('craft_potential_hotkey') or DEFAULT_CRAFT_POTENTIAL_HOTKEY)}"
        )
    ...
    ctk.CTkLabel(
        ...
        text=(
            "Hotkey disclaimer: Windows can conflict with combinations such as "
            "Ctrl+1 and Ctrl+Down. If a hotkey misfires, rebind it to "
            "Ctrl+Shift+1 (or similar)."
        ),
        text_color="#9AA7B4",
        font=_FONT_HELPER,
        justify="left",
    ...
    def _make_hotkey_commit(entry_obj, key: str, rebind):
        def _commit() -> bool:
            ...
            if state.config.get(key, "") == value:
                return True  # nothing changed; avoid needless rebind/disk write
            state.config[key] = value
            try:
                rebind(value)
            except Exception:
                logging.exception("Rebinding hotkey %s failed", key)
            ok = _persist_settings_to_disk(key)
...
def _verify_post_swap_update() -> None:
    """At startup, confirm a just-applied swap actually advanced the version."""
    try:
        failed = updater.verify_post_swap_update(
            updater.default_update_stage_dir(), __version__
        )
    except Exception:
...
def _apply_staged_update(outcome: "updater.UpdateOutcome") -> bool:
    ...
    try:
        ...
        updater.launch_app_update_after_exit(outcome.app_package_dir)
        _status().present("checking", f"Applying update v{outcome.app_version}; restarting…")
        if root is not None and root.winfo_exists():
...
class _LocalTestUpdateResponse:
...
class _LocalTestUpdateSession:
    """Tiny requests-like session for build_app's local updater test artifacts."""
    ...
    def get(self, url: str, **_kwargs):
        ...
        if parsed.netloc.lower() != "local.stashsage.test":
            raise ValueError(f"unsupported local test update URL: {url}")
        name = Path(parsed.path).name
        if name == self.manifest_path.name:
            return _LocalTestUpdateResponse(self.manifest_path)
        return _LocalTestUpdateResponse(self.root / name)
...
def _on_quit(icon, item):
    ...
    if root is not None:
        try:
            ...
            logging.debug("root quit failed", exc_info=True)
    sys.exit()
