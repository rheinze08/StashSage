...
def _xgb_bar_text(payload: Mapping[str, Any]) -> str:
    ...
    category = str(payload.get("category_title") or "item").strip().title()
    return (
        f"Model #1 \u00b7 XGB  |  Prediction {_priced(payload.get('display_value'), payload)}"
        f"  |  Relative {label} value  |  {category}"
    )
...
def _knn_bar_text(
    ...
    if isinstance(summary, Mapping):
        ...
        if isinstance(count, (int, float)):
            parts.append(f"{int(count)} listings")
        for key, label in (("median", "Median Prediction"), ("mean", "Mean Prediction")):
            try:
                if summary.get(key) is not None:
                    parts.append(f"{label} {_priced(summary[key], payload)}")
            except (TypeError, ValueError):
                pass
    prices = payload.get("listing_prices")
    if isinstance(prices, (list, tuple)):
        values = [str(value).strip() for value in prices[:10] if str(value).strip()]
        if values:
            parts.append("Prices " + ", ".join(values))
    return "  |  ".join(parts)
...
class PredictionPresenter:
    ...
    def _shell(self, *, hidden: bool = False) -> ctk.CTkToplevel:
        ...
        self._make_nonactivating(self.overlay)
    ...
    def _hidden_result_shell(self) -> ctk.CTkToplevel:
        ...
        self._make_nonactivating(result)
    ...
    def _make_nonactivating(self, window) -> None:
        """Keep PoE foregrounded while this presenter surface is visible."""
    ...
    def show_loading(
        ...
        *,
        ...
        hint: str = "Click outside the panel to close",
    ...
    def _model_summary_row(
        ...
        label = str(payload.get("bucket_label") or "").capitalize()
        colour = BUCKET_COLOURS.get(label.lower(), BAR_COLOUR)
        model_one = ctk.CTkFrame(layout, fg_color=colour, corner_radius=8)
        ...
        model_two = ctk.CTkFrame(layout, fg_color="#294354", corner_radius=8)
        model_two.grid(row=2, column=1, sticky="ew", padx=8, pady=(4, 0))
        ctk.CTkLabel(
            model_two,
            text=_knn_bar_text(payload, summary),
            font=("Segoe UI", 15, "bold"),
            text_color="#E3F2FA",
        ).pack(fill="x", padx=8, pady=6)
...
class CraftPresenter:
    ...
    def __init__(self, root, monitor_rect, event_queue, on_closed, session_id: int = 0) -> None:
        self.host = PredictionPresenter(root, 0, on_closed, monitor_rect)
    ...
    def show_progress(self, done: int | None = None, total: int | None = None) -> None:
        ...
        self.host.show_loading(
            ...
            detail=text,
            hint="Click outside the panel to cancel",
            reuse_shell=True,
        )
