...
from poe2trade import chaos_exalt, divine_exalt, annul_exalt, poe2trade_root
...
def icon_label(
    ...
    *,
    ...
    for category in ("Body_Armour", "Helmet", "Boots", "Gloves", "Focus", "Shield", "Buckler", "Ring", "Amulet", "Belt", "Wand", "Sceptre", "Quiver"):
...
def price_to_exalt(price: float, currency: str | None) -> float:
    ...
    if currency in {
        ...
        return float(price) * annul_exalt
    return float(price)
...
def price_string(row: pd.Series) -> tuple[str, float] | None:
    ...
    try:
        ...
        annul = int(round(float(e_value) / max(annul_exalt, 1e-9)))
    except Exception:
        return None
    return (f"{exalt}e/{chaos}c/{divine}d/{annul}a", float(e_value))
...
def triple(price_in_exalts: float) -> str:
    try:
        ...
        annul = int(round(float(price_in_exalts) / max(annul_exalt, 1e-9)))
        return f"{exalt}e/{chaos}c/{divine}d/{annul}a"
    except Exception:
