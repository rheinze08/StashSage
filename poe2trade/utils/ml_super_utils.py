...
def _compact_scoring_json(json_path: Path) -> bool:
    ...
    try:
        ...
        print(f"[DEBUG] Skipping {json_path.name}: load failed ({exc!r})")
        return False
...
def _maybe_compact_scoring_jsons() -> None:
    """Ensure scoring JSON sidecars only retain index/score/bucket fields."""
    ...
    compacted = 0
    for json_path in model_dir.glob("*_scoring.json"):
        try:
            if _compact_scoring_json(json_path):
                compacted += 1
        except Exception as exc:
            print(f"[DEBUG] Failed to compact {json_path.name}: {exc!r}")

    if compacted:
        print(f"[DEBUG] Compacted {compacted} scoring JSON file(s) to index/pred/bucket columns")
