...
def generate_prediction_vs_listings_chart(
    ...
    *,
    title: str = "XGB & KNN Price Prediction Range",
    width: float = 9.4,
    height: float = 3.1,
) -> io.BytesIO:
    ...
    ax.scatter(values, y, s=75, color="#377EAA", edgecolor="#173548", linewidth=1.0, zorder=3, label="Similar Items")
    ...
    ax.axvline(median, color="#6E4EA1", linewidth=2, linestyle="--", label=f"KNN Median {median:.0f}e")
    ax.axvline(mean, color="#D0882C", linewidth=2, linestyle=":", label=f"KNN Mean {mean:.0f}e")
    ...
    if prediction is not None and isinstance(prediction, (int, float)) and np.isfinite(float(prediction)) and float(prediction) > 0:
        ...
        ax.axvline(value, color="#C24B48", linewidth=2.6, label=f"XGB Prediction {value:.0f}e")
        ax.annotate(f"{value:.0f}e", (value, .50), xytext=(5, 0),
                    textcoords="offset points", ha="left", va="center",
                    fontsize=9, fontweight="bold", color="#C24B48",
                    bbox=dict(facecolor="white", edgecolor="none", alpha=.85, pad=1.5))
        predictions.append(value)
    ax.axvspan(min(predictions), max(predictions), color="#708DA5", alpha=.24,
               linewidth=0, zorder=0, label="Prediction Range")
    low = min(float(values.min()), min(predictions))
    high = max(float(values.max()), max(predictions))
...
def generate_predicted_overlay_with_marker(
    ...
    *,
    ...
    if all_pred.size:
        ...
        # Equal distances represent equal exalt amounts, including long tails.
