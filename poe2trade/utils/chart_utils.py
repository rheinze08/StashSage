...
• Fixed canvas preserved (6×4 in @ 96 dpi).
"""
    ...
    *,
    ...
    if all_pred.size:
        ...
        for lbl in buckets:
            ...
            if len(vals):
                cnts, _ = np.histogram(vals, bins=edges)
                dens = cnts / (total_n * widths)
                y_edges = np.r_[dens, dens[-1]]
                ax.fill_between(edges, y_edges, step='post', alpha=0.30, color=colours.get(lbl))
                ax.step(edges, y_edges, where='post', linewidth=1.8, color=colours.get(lbl), label=lbl)
    ...
    if marker_value is not None and isinstance(marker_value, (int, float)):
        try:
            ...
            ymax = ax.get_ylim()[1]
            ax.text(mv, ymax * 0.95, "Your item", rotation=90,
...
def _canon_cur(s: str) -> str:
    s = (s or "").strip().lower()
    if s.startswith("ex"):     return "exalted"
    if s.startswith("chaos") or s.startswith("c"): return "chaos"
    if s.startswith("div"):    return "divine"
    return s or "chaos"
