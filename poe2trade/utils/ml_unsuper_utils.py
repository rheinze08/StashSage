...
def call_ml(
    ...
    *,
    ...
    pd.DataFrame or None
        Overlay data of up to `top` nearest neighbours after filters, or
    ...
    has_feature_filters = bool(where)
    has_overlay_filters = bool(where_overlay)
    has_per_call_price  = (min_price is not None)

    if not (has_feature_filters or has_overlay_filters or has_per_call_price):
        # ── FAST PATH: use fitted NN, then apply global price filter (same as previous behaviour) ──
        n_query = min(top, len(X_all))
        print(f"[DEBUG] call_ml_unsuper: running KNN (k={n_query}) [fast path]")
        dist, ix = nn.kneighbors(qv, n_neighbors=n_query)
        dists = dist.flatten()
        idxs  = ix.flatten()

        # price filter (post-NN to preserve legacy semantics)
    ...
    fm = _build_feature_mask(X_all, feat_cols, where)
    if fm is not None:
        cand &= fm

    # 2) overlay-based filters (attributes that live only in overlay_df)
    om = _build_overlay_mask(overlay_df, row_indices, where_overlay)
    if om is not None:
        cand &= om
